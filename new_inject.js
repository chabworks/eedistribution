...
// for 200
window.sub_cat_urls=["https://www.eedistribution.com/newproduct.asp"];
...
window.call_api_data = function (input_key,trycounter=0){
	...
	jQuery.ajax({
		...
		var dom_nodes = jQuery(jQuery.parseHTML(ajax_data));
			...
			to_be_push['new_product_type'] = input_key['new_product_type'];
			to_be_push['ref_date'] = input_key['ref_date'];
...
page_url=window.sub_cat_urls[window.category_pointer]+'';
	jQuery.ajax({
		...
		// var main_pg_html = jQuery(jQuery.parseHTML(mainpage_res));
		var dom_nodes_r = jQuery(jQuery.parseHTML(mainpage_res));

		// dom_nodes_r.find('td.SmBlack table[cellspacing="0"]:contains(NEW ARRIVALS/HOT OFF THE TRUCK):first tbody tr:first').remove()
		// dom_nodes_r.find('td.SmBlack table[cellspacing="0"]:contains(NEW ARRIVALS/HOT OFF THE TRUCK):first')

		//Table 1

		// if (dom_nodes_r.find('td.SmBlack table[cellspacing="0"]:contains(NEW ARRIVALS/HOT OFF THE TRUCK):first tr td a').length) {
		// 	dom_nodes_r.find('td.SmBlack table[cellspacing="0"]:contains(NEW ARRIVALS/HOT OFF THE TRUCK):first tr td a').forEach(function(e){
		// 	  var page_link = '';
		// 	  if(jQuery(e).attr('href').includes("prodinfo.asp?number=")){

		// 	    var page_link = "https://www.eedistribution.com/"+jQuery(e).attr('href');

		// 	    // if(window.input_data.indexOf(page_link) === -1){
		// 	      window.input_data.push(page_link);
		// 	      window.input_data.push({"url":"","new_product_type":"","ref_date":""});
		// 	    // }
		...
		// if (dom_nodes_r.find('td.SmBlack table[cellspacing="0"]:contains(TOP 50 BESTSELLERS):first tr td a').length) {
		// 	dom_nodes_r.find('td.SmBlack table[cellspacing="0"]:contains(TOP 50 BESTSELLERS):first tr td a').forEach(function(e){
		// 	  var page_link = '';
		// 	  if(jQuery(e).attr('href').includes("prodinfo.asp?number=")){
		...
		dom_nodes_r.find('table:contains("Looking for New Pre-Orders")').next().find('a[href*="prodinfo.asp?number="]').each(function(i,e){
			var currentitemdate=jQuery(e).closest('tr').last().prevAll().find('.SmBlueBold').last().text().trim()
			// console.log(currentitemdate)
			...
			var page_link = "https://www.eedistribution.com/"+jQuery(e).attr('href');
			    ...
			    // if(window.input_data.indexOf(page_link) === -1){
			      ...
			      if (currentitemdate) {
			      	new_product_type='NEW ARRIVALS';
			      }
			      else
			      	new_product_type='TOP 50 BESTSELLERS';

			      window.input_data.push({"url":page_link,"new_product_type":new_product_type,"ref_date":currentitemdate});
			    // }
		})
